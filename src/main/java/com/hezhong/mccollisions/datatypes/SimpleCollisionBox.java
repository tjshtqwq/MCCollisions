...
public class SimpleCollisionBox implements CollisionBox {
     ...
     */
    public Vector3d intersectsRay(Vector3d origin, Vector3d direction, float minDist, float maxDist) {
        Vector3d invDir = new Vector3d(1f / direction.getX(), 1f / direction.getY(), 1f / direction.getZ());

        boolean signDirX = invDir.getX() < 0;
        boolean signDirY = invDir.getY() < 0;
        boolean signDirZ = invDir.getZ() < 0;

        double tmin = ((signDirX ? maxX : minX) - origin.getX()) * invDir.getX();
        double tmax = ((signDirX ? minX : maxX) - origin.getX()) * invDir.getX();
        double tymin = ((signDirY ? maxY : minY) - origin.getY()) * invDir.getY();
        double tymax = ((signDirY ? minY : maxY) - origin.getY()) * invDir.getY();
        ...
        double tzmin = ((signDirZ ? maxZ : minZ) - origin.getZ()) * invDir.getZ();
        double tzmax = ((signDirZ ? minZ : maxZ) - origin.getZ()) * invDir.getZ();
        ...
        return tmin < maxDist && tmax > minDist
                ? new Vector3d(origin.getX() + direction.getX() * tmin, origin.getY() + direction.getY() * tmin, origin.getZ() + direction.getZ() * tmin)
                : null;
    }
    ...
    public static Iterable<Vector3i> betweenClosed(int xStart, int yStart, int zStart, int xEnd, int yEnd, int zEnd) {
        ...
        return () -> new Iterator<>() {
            ...
            public boolean hasNext() {
                return this.index < totalVectors;
            }
    ...
    public static Iterable<Vector3i> betweenCornersInDirection(int x1, int y1, int z1, int x2, int y2, int z2, Vector3d directionVector) {
        ...
        List<Axis> axisOrder = axisStepOrder(directionVector);
        Axis primaryAxis = axisOrder.get(0);
        Axis secondaryAxis = axisOrder.get(1);
        Axis tertiaryAxis = axisOrder.get(2);
    ...
    private static List<Axis> axisStepOrder(Vector3d vector) {
