...
import com.hezhong.mccollisions.blocks.DoorHandler;
import com.hezhong.mccollisions.blocks.DynamicChest;
import com.hezhong.mccollisions.blocks.DynamicChorusPlant;
import com.hezhong.mccollisions.blocks.DynamicStair;
import com.hezhong.mccollisions.blocks.PistonBaseCollision;
import com.hezhong.mccollisions.blocks.PistonHeadCollision;
import com.hezhong.mccollisions.blocks.TrapDoorHandler;
import com.hezhong.mccollisions.blocks.connecting.DynamicCollisionFence;
import com.hezhong.mccollisions.blocks.connecting.DynamicCollisionPane;
import com.hezhong.mccollisions.blocks.connecting.DynamicCollisionWall;
...
import com.hezhong.mccollisions.datatypes.DynamicCollisionBox;
...
public enum CollisionData implements CollisionFactory {
    LAVA((context, version, block, x, y, z) -> {
        if (context.isAboveLava() && context.isRidingStrider()) {
            if (block.getLevel() == 0) {
    ...
    BREWING_STAND((context, version, block, x, y, z) -> {
        int base = 0;
    ...
    BAMBOO((context, version, block, x, y, z) -> {
    ...
    COMPOSTER((context, version, block, x, y, z) -> {
        double height = 0.125;
    ...
    ANVIL((context, version, data, x, y, z) -> {
    ...
    SLAB((context, version, data, x, y, z) -> {
    ...
    WALL_SKULL((context, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    PIGLIN_WALL_HEAD((context, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    HOPPER((context, version, data, x, y, z) -> {
        if (version.isNewerThanOrEquals(ClientVersion.V_1_13)) {
    ...
    CAKE((context, version, data, x, y, z) -> {
        double height = 0.5;
    ...
    COCOA_BEANS((context, version, data, x, y, z) -> getCocoa(version, data.getAge(), data.getFacing()), StateTypes.COCOA),

    STONE_CUTTER((context, version, data, x, y, z) -> {
        if (version.isOlderThanOrEquals(ClientVersion.V_1_13_2))
            return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);
    ...
    BELL((context, version, data, x, y, z) -> {
        if (version.isOlderThanOrEquals(ClientVersion.V_1_13_2))
            return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);
    ...
    SCAFFOLDING((context, version, data, x, y, z) -> {
        // ViaVersion replacement block - hay block
        if (version.isOlderThanOrEquals(ClientVersion.V_1_13_2))
            return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);

        if (context.lastY() > y + 1 - 1e-5 && !context.isSneaking()) {
            return new ComplexCollisionBox(5,
    ...
    CAMPFIRE((context, version, data, x, y, z) -> {
    ...
    LANTERN((context, version, data, x, y, z) -> {
        if (version.isOlderThanOrEquals(ClientVersion.V_1_12_2))
            return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);
    ...
    LECTERN((context, version, data, x, y, z) -> {
        if (version.isOlderThanOrEquals(ClientVersion.V_1_13_2))
            return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);
    ...
    HONEY_BLOCK((context, version, data, x, y, z) -> {
    ...
    GRINDSTONE((context, version, data, x, y, z) -> {
    ...
    CHAIN_BLOCK((context, version, data, x, y, z) -> {
        if (version.isOlderThan(ClientVersion.V_1_16)) {
            // viaversion replacement - iron bars
            return PANE.fetch(context, version, data, x, y, z);
        }
    ...
    FENCE_GATE((context, version, data, x, y, z) -> {
        if (data.isOpen())
            return NoCollisionBox.INSTANCE;
    ...
    SNOW((context, version, data, x, y, z) -> {
    ...
    FRAME((context, version, data, x, y, z) -> {
    ...
    CARPET((context, version, data, x, y, z) -> {
    ...
    PALE_MOSS_CARPET((context, version, data, x, y, z) -> {
        if (!data.isBottom()) {
            return NoCollisionBox.INSTANCE;
        }

        if (version.isOlderThan(ClientVersion.V_1_21_2)) {
            return MOSS_CARPET.fetch(context, version, data, x, y, z);
        }
    ...
    FARMLAND((context, version, data, x, y, z) -> {
        ...
        if (version == ClientVersion.V_1_10) {
            if (Math.abs(context.y() % 1.0) < 0.001) {
                return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);
            }
    ...
    GRASS_PATH((context, version, data, x, y, z) -> {
    ...
    LILYPAD((context, version, data, x, y, z) -> {
        // Boats break lilypads client sided on 1.12- clients.
        if (context.inVehicle() && context.isRidingBoat() && version.isOlderThanOrEquals(ClientVersion.V_1_12_2))
            return NoCollisionBox.INSTANCE;
    ...
    BED((context, version, data, x, y, z) -> {
    ...
    END_ROD((context, version, data, x, y, z) -> getEndRod(version, data.getFacing()), Materials.getRods().toArray(new StateType[0])),

    CAULDRON((context, version, data, x, y, z) -> {
    ...
    PICKLE((context, version, data, x, y, z) -> getPicklesBox(version, data.getPickles()), StateTypes.SEA_PICKLE),

    TURTLEEGG((context, version, data, x, y, z) -> {
    ...
    CONDUIT((context, version, data, x, y, z) -> {
    ...
    WALL_SIGN((context, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    WALL_FAN((context, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    CORAL_PLANT((context, version, data, x, y, z) -> new HexCollisionBox(2.0D, 0.0D, 2.0D, 14.0D, 15.0D, 14.0D), Stream.concat(
    ...
    STONE_PRESSURE_PLATE((context, version, data, x, y, z) -> {
    ...
    WOOD_PRESSURE_PLATE((context, version, data, x, y, z) -> {
        if (data.isPowered()) { // Pressed
            return new HexCollisionBox(1.0D, 0.0D, 1.0D, 15.0D, 0.5D, 15.0D);
        }
    ...
    OTHER_PRESSURE_PLATE((context, version, data, x, y, z) -> {
    ...
    TRIPWIRE((context, version, data, x, y, z) -> {
        if (data.isAttached()) {
    ...
    TRIPWIRE_HOOK((context, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    WALL_TORCH((context, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    // 1.17 blocks
    CANDLE((context, version, data, x, y, z) -> {
    ...
    CANDLE_CAKE((context, version, data, x, y, z) -> {
    ...
    DECORATED_POT((context, version, data, x, y, z) -> {
    ...
    BIG_DRIPLEAF((context, version, data, x, y, z) -> {
    ...
    POINTED_DRIPSTONE((context, version, data, x, y, z) -> {
        if (version.isOlderThan(ClientVersion.V_1_17))
    ...
    POWDER_SNOW((context, version, data, x, y, z) -> {
        if (version.isOlderThanOrEquals(ClientVersion.V_1_16_4))
            return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);

        // If fall distance greater than 2.5, 0.899999 box
        if (context.fallDistance() > 2.5) {
            // TODO: this is technically incorrect (1.21.4)
            return context.clientVersion() == ClientVersion.V_1_21_4 ?
                    new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true)
                    : new SimpleCollisionBox(0.0, 0.0, 0.0, 1.0, 0.9, 1.0, false);
        }

        if (context.lastY() > y + 1 - 1e-5 && context.hasLeatherBoots() && !context.isSneaking() && !context.inVehicle())
            return new SimpleCollisionBox(0, 0, 0, 1, 1, 1, true);
    ...
    NETHER_PORTAL((context, version, data, x, y, z) -> {
        if (data.getAxis() == Axis.X) {
    ...
    AZALEA((context, version, data, x, y, z) -> new ComplexCollisionBox(2,
    ...
    AMETHYST_CLUSTER((context, version, data, x, y, z) -> getAmethystBox(version, data.getFacing(), 7, 3), StateTypes.AMETHYST_CLUSTER),

    SMALL_AMETHYST_BUD((context, version, data, x, y, z) -> getAmethystBox(version, data.getFacing(), 3, 4), StateTypes.SMALL_AMETHYST_BUD),

    MEDIUM_AMETHYST_BUD((context, version, data, x, y, z) -> getAmethystBox(version, data.getFacing(), 4, 3), StateTypes.MEDIUM_AMETHYST_BUD),

    LARGE_AMETHYST_BUD((context, version, data, x, y, z) -> getAmethystBox(version, data.getFacing(), 5, 3), StateTypes.LARGE_AMETHYST_BUD),

    MUD_BLOCK((context, version, data, x, y, z) -> {
    ...
    MANGROVE_PROPAGULE_BLOCK((context, version, data, x, y, z) -> {
        if (!data.isHanging()) {
    ...
    SCULK_SHRIEKER((context, version, data, x, y, z) -> {
    ...
    SNIFFER_EGG((context, version, data, x, y, z) -> {
    ...
    PITCHER_CROP((context, version, data, x, y, z) -> {
        if (version.isNewerThan(ClientVersion.V_1_19_4)) {
    ...
    WALL_HANGING_SIGNS((context, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    DRIED_GHAST((context, version, data, x, y, z) -> {
        if (context.clientVersion().isNewerThan(ClientVersion.V_1_21_5)) {
        ...
        } else if (context.clientVersion().isNewerThan(ClientVersion.V_1_12_2)) {
        ...
        } else if (context.clientVersion().isNewerThan(ClientVersion.V_1_8)) {
    ...
    SHELF((context, version, data, x, y, z) -> {
    ...
    COPPER_GOLEM_STATUE((context, version, data, x, y, z) -> {
        if (version.isOlderThan(ClientVersion.V_1_21_9)) {
    ...
    public CollisionBox getMovementCollisionBox(CollisionContext context, ClientVersion version, WrappedBlockState block, int x, int y, int z) {
        return fetch(context, version, block, x, y, z).offset(x, y, z);
    }

    public CollisionBox getMovementCollisionBox(CollisionContext context, ClientVersion version, WrappedBlockState block) {
        if (this.box != null)
            return this.box.copy();

        return new DynamicCollisionBox(context, version, dynamic, block);
    }

    @Override
    public CollisionBox fetch(CollisionContext context, ClientVersion version, WrappedBlockState block, int x, int y, int z) {
        return box != null ? box.copy() : new DynamicCollisionBox(context, version, dynamic, block);
    }
}
