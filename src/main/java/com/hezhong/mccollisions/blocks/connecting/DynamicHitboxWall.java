...
public class DynamicHitboxWall extends DynamicConnecting implements HitBoxFactory {
    @Override
    public CollisionBox fetch(CollisionContext context, StateType heldItem, ClientVersion version, WrappedBlockState state, boolean isTargetBlock, int x, int y, int z) {
        int[] connections = getConnections(context, version, state, x, y, z);
    ...
    private int[] getConnections(CollisionContext context, ClientVersion version, WrappedBlockState state, int x, int y, int z) {
        int north, south, west, east, up;

        if (isModernServer(context)) {
    ...
    private boolean isModernServer(CollisionContext context) {
        return context.serverVersion().isNewerThan(ServerVersion.V_1_12_2);
    }
    ...
    public boolean checkCanConnect(CollisionContext context, WrappedBlockState state, StateType one, StateType two, BlockFace direction) {
        return BlockTags.WALLS.contains(one) ||
                CollisionData.getData(one).getMovementCollisionBox(context, context.clientVersion(), state, 0, 0, 0).isSideFullBlock(direction);
    }
}
