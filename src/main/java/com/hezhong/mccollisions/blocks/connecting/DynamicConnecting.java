...
public class DynamicConnecting {
    ...
    public boolean connectsTo(CollisionContext context, ClientVersion v, int currX, int currY, int currZ, BlockFace direction) {
        WrappedBlockState targetBlock = context.getBlock(currX + direction.getModX(), currY + direction.getModY(), currZ + direction.getModZ());
        WrappedBlockState currBlock = context.getBlock(currX, currY, currZ);
        ...
        if (target == StateTypes.BARRIER)
            return context.clientVersion().isOlderThanOrEquals(ClientVersion.V_1_7_10) ||
                    context.clientVersion().isNewerThanOrEquals(ClientVersion.V_1_9) &&
                            context.clientVersion().isOlderThanOrEquals(ClientVersion.V_1_11_1);
        ...
        if (BlockTags.STAIRS.contains(target)) {
            ...
            if (v.isOlderThan(ClientVersion.V_1_12) || (context.serverVersion().isOlderThanOrEquals(ServerVersion.V_1_11) && v.isNewerThanOrEquals(ClientVersion.V_1_13)))
                return false;
            ...
            return checkCanConnect(context, targetBlock, target, fence, direction);
        }
    }
