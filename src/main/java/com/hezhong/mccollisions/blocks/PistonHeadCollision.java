...
public class PistonHeadCollision implements CollisionFactory {
    ...
    public CollisionBox fetch(CollisionContext context, ClientVersion version, WrappedBlockState block, int x, int y, int z) {
        ...
        double longAmount = context.serverVersion().isNewerThanOrEquals(ServerVersion.V_1_13) && block.isShort() ? 0 : 4;
        ...
        if (version.isOlderThanOrEquals(ClientVersion.V_1_12_2) || context.serverVersion().isOlderThanOrEquals(ServerVersion.V_1_12_2))
            longAmount = 4;


        // 1.8 and 1.7 clients always have "short" piston collision boxes
        // Apply last to overwrite other long amount setters
        if (version.isOlderThan(ClientVersion.V_1_9) || context.serverVersion().isOlderThan(ServerVersion.V_1_9))
            longAmount = 0;
