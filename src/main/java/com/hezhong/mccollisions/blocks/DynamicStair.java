...
public class DynamicStair implements CollisionFactory {
    ...
    public static EnumShape getStairsShape(CollisionContext context, WrappedBlockState originalStairs, int x, int y, int z) {
        BlockFace facing = originalStairs.getFacing();
        WrappedBlockState offsetOne = context.getBlock(x + facing.getModX(), y + facing.getModY(), z + facing.getModZ());
        ...
        if (Materials.isStairs(offsetOne.getType()) && originalStairs.getHalf() == offsetOne.getHalf()) {
            ...
            if (isDifferentAxis(facing, enumfacing1) && canTakeShape(context, originalStairs, x + enumfacing1.getOppositeFace().getModX(), y + enumfacing1.getOppositeFace().getModY(), z + enumfacing1.getOppositeFace().getModZ())) {
        ...
        WrappedBlockState offsetTwo = context.getBlock(x + facing.getOppositeFace().getModX(), y + facing.getOppositeFace().getModY(), z + facing.getOppositeFace().getModZ());
        ...
        if (Materials.isStairs(offsetTwo.getType()) && originalStairs.getHalf() == offsetTwo.getHalf()) {
            ...
            if (isDifferentAxis(facing, enumfacing2) && canTakeShape(context, originalStairs, x + enumfacing2.getModX(), y + enumfacing2.getModY(), z + enumfacing2.getModZ())) {
    ...
    private static boolean canTakeShape(CollisionContext context, WrappedBlockState stairOne, int x, int y, int z) {
        WrappedBlockState otherStair = context.getBlock(x, y, z);
    ...
    public CollisionBox fetch(CollisionContext context, ClientVersion version, WrappedBlockState block, int x, int y, int z) {
        ...
        if (context.serverVersion().isNewerThanOrEquals(ServerVersion.V_1_13)
            ...
            EnumShape shape = getStairsShape(context, block, x, y, z);
