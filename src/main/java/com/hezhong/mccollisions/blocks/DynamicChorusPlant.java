...
public class DynamicChorusPlant implements CollisionFactory {
    ...
    public CollisionBox fetch(CollisionContext context, ClientVersion version, WrappedBlockState block, int x, int y, int z) {
        ...
        if (version.isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
            return getLegacyBoundingBox(context, version, x, y, z);
        }
        ...
        if (context.serverVersion().isNewerThanOrEquals(ServerVersion.V_1_13)) {
            ...
            directions = getLegacyStates(context, version, x, y, z);
        }
        // Player is 1.13+ on 1.13+ server
        return modernShapes[getAABBIndex(directions)].copy();
    }

    public CollisionBox getLegacyBoundingBox(CollisionContext context, ClientVersion version, int x, int y, int z) {
        Set<BlockFace> faces = getLegacyStates(context, version, x, y, z);
    ...
    public Set<BlockFace> getLegacyStates(CollisionContext context, ClientVersion version, int x, int y, int z) {
        ...
        StateType downBlock = context.getBlockType(x, y - 1, z);
        StateType upBlock = context.getBlockType(x, y + 1, z);
        StateType northBlock = context.getBlockType(x, y, z - 1);
        StateType eastBlock = context.getBlockType(x + 1, y, z);
        StateType southBlock = context.getBlockType(x, y, z + 1);
        StateType westBlock = context.getBlockType(x - 1, y, z);
